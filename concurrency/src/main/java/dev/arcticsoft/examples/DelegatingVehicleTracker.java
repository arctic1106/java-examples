 ...
 *
 * This class does not use any explicit synchronization; all access to state is
 * managed by ConcurrentHashMap, and all keys and values of the Map are
 * immutable.
 *
 */
public class DelegatingVehicleTracker {
    ...
    /**
     * This method returns an unmodifiable but "live" view of the vehicle
     * locations. This means if thread A calls getLocations() and thread B later
     * modifies the location of some of the points, those changes are reflected
     * in the map returned to thread A.
     *
     * This can be benefit (more up-to-date data) or liability (potentially
     * inconsistent view of data), depending on your requirements.
     */
    ...
    /**
     * Point is thread-safe because it is immutable. Immutable values can be
     * freely shared and published, so we no longer need to copy the locations
     * when returning them.
     */
     ...
     *
     * Since the contents of the Map is are immutable, only the structure of the
     * Map, not the contents, must be copied.
     */
