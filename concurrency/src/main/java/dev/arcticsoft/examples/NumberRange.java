 ...
 *
 * Both "setLower" and "setUpper" are check-then-act sequences (compound
 * actions), but they do not use sufficient locking to make them atomic.
 *
 * An example of an invalid state o the range object:
 *
 * If the number range holds (0,10), and one thread calls "setLower(5)" while
 * another thread calls "setUpper(4)", with some unlucky timing both will pass
 * the checks in the setters and both modifications will be applied. The result
 * is that the range now holds (5,4) - an invalid state.
 *
 ...
 *
 * It must also avoid publishing "lower" and "upper" to prevent clients from
 ...
 *
 * The problem that prevented NumberRange from being thread-safe even though its
 * state components were thread-safe is very similar to one of the rules about
 * volatile variables:
 *
