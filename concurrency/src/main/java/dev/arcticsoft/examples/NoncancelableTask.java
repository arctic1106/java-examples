        ...
        try {
            while (true) {
                try {
                    ...
                    /**
                     * WARN: Setting the interrupted status too early could
                     * result in an infinite loop, because most interruptible
                     * blocking methods check the interrupted status on entry
                     * and throw InterruptedException immediately if it is set.
                     *
                     * Calling Thread.currentThread().interrupt() here would set
                     * the status=true which would cause calls to queue.take to
                     * throw InterruptedException in the next rounds in the loop
                     * forever.
                     */
                    interrupted = true;
