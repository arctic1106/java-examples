 ...
 *
 * Another way to convince producer-consumer service to shut down is with a
 * poison pill: a recognizable object placed on the queue that means "when you
 * get this, stop".
 *
 * IndexingService shows a single-producer, single-consumer version of the
 * desktop search application in link {@link ProducerConsumer}
 *
 * Poison pills work only when the number of producers and consumers is known.
 *
 * The approach in IndexingService can be extended to multiple producers by
 * having each producer place a pill on the queue and having the consumer stop
 * when it receives N_producers pills.
 *
 * It can be extended to multiple consumers by having each producer place
 * N_consumers pills on the queue, tough this can get unwieldy with large number
...
public class IndexingService {
    ...
    public IndexingService(final File root, final FileFilter fileFilter) {
        this.filter = (final File f) -> f.isDirectory() || fileFilter.accept(f);
