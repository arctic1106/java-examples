 ...
 * intrinsic lock.
 *
 * The design process of a thread-safe class should include these three basic
 * elements:
 *
 ...
 *
 * This Counter class has only one field, so the "value" field comprises its
 * entire state.
 *
 * You cannot ensure thread-safety without understanding an object's invariants
 * and
 * postconditions. Constraints on the valid values or state transitions for
 * state variables
 * can create atomicity and encapsulation requirements.
 *
 * When multiple variables participate in an invariant, the lock that guards
 * them must be held
...
public final class Counter {
    ...
    /**
     * Operations may have postconditions that identify certain state state transitions as invalid.
     * If the current state of the counter is 17, the only valid next state is 18.
     */
