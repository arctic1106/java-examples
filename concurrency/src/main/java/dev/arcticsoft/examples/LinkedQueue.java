 ...
 * pointer of the current last element, and the tail pointer.
 */
public class LinkedQueue<E> {
    ...
    /**
     * Inserting a new element involves updating two pointers. The first links
     * the new node to the end of the list by updating the "next" pointer of the
     * current last element; the second swings the tail pointer around to point
     * to the new last element. Between these two operations, the queue is in
     * the intermediate state.
     *
     * The key observation that enables both of the required tricks is that if
     * the queue is in the quiescent (normal) state, the "next" field of the
     * link node pointed to by tail is null, and if it is in the intermediate
     * state, tail.next is non-null.
     *
     ...
     * until it finds the queue in quiescent state so it can begin its own
     * insertion.
     *
     ...
     * thread can just reload the tail pointer and try again.
     *
     * If D fails, the inserting thread returns anyway rather than retrying the
     * CAS, because no retry is needed - another thread has already finished the
     * job in its step B!
     */
