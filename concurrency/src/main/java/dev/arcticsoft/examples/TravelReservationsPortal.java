 ...
 * service, consulting a database, performing an EDI transaction. Rather than
 * have the response time for the page driven by the slowest response, it may be
...
public class TravelReservationsPortal {
    ...
    public List<TravelQuote> getRankedTravelQuotes(
            ...
            final Set<TravelCompany> companies,
            final Comparator<TravelQuote> ranking,
            final long time,
            final TimeUnit unit
        ...
        /**
         * It would be easy enough to create N tasks, submit them to a thread
         * pool, retain the Futures, and use a timed get to fetch each result
         * sequentially via its Future, but there is an even easier way -
         * invokeAll().
         */
        final List<Future<TravelQuote>> futures = exec.invokeAll(tasks, time, unit);

        /**
         * The invokeAll method takes a collection of tasks and returns a
         * collection of Futures. The two collections have identical structures;
         * invokeAll adds the Futures to the returned collection in order
         * imposed by the task collection's iterator, thus allowing the caller
         * to associate a Future with the Callable it represents.
         */
