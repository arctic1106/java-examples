...
public class InduceLockOrder {
    ...
    public void transferMoney(final Account fromAcct, final Account toAcct, final DollarAmount amount)
         ...
         * In the rare case that to objects have the same hashcode, we must use
         * an arbitrary means of ordering the lock acquisitions, an this
         * reintroduces the possibility of deadlock. To prevent inconsistent
         * lock ordering in this case, a third "tie breaking" lock is used.
         *
         * By acquiring the tie-breaking lock before acquiring either Account
         * lock, we ensure that only one thread at a time performs the risky
         * task of acquiring two locks in an arbitrary order, eliminating the
