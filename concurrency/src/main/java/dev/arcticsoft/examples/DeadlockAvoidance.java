 ...
 *
 * The sleep time has a fixed component and a random component to reduce the
 * likelihood of livelock. If the locks cannot be acquired within te specified
 * time, the "transferMoney" method returns a failure status so that the
 * operation can fail gracefully.
 */
    ...
    public boolean transferMoney(
        ...
        while (true) {
            if (fromAcct.lock.tryLock()) {
                 ...
                 * ReentrantLock as a blanket substitute for "synchronized": it
                 * is more "dangerous" because it doesn't automatically clean up
                 * the lock when control leaves the guarded block.
                 *
