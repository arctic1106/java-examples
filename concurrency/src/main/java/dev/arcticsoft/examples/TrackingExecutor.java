 ...
 *
 * TrackingExecutor shows a technique for determining which tasks were in
 * progress at shutdown time (as there is no way of knowing the state of the
 * tasks in progress at shutdown time unless the tasks themselves perform some
 * sort of checkpointing).
 *
 * By encapsulating an ExecutorService and instrumenting "execute" (and
 * similarly submit) to remember which tasks were cancelled after shutdown,
...
public class TrackingExecutor extends AbstractExecutorService {
    ...
    /**
     * After the executor terminates, getCancelledTasks returns the list of
     * cancelled tasks. In order for this technique to work, the tasks must
     * preserve the thread's interrupted status when they return, which well
     * behaved tasks will do anyway.
     */
