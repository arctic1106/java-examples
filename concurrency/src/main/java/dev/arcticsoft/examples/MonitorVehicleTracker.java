 ...
 * application where it might be shared by a view thread and multiple updater
 * threads.
 *
 * Since the view thread and the updater threads will access the data model
 * concurrently, it must be thread-safe.
 *
 ...
 * need to return vehicle locations to callers, the appropriate values are
...
public class MonitorVehicleTracker {
    ...
    public MonitorVehicleTracker(final Map<String, MutablePoint> locations) {
        /**
         * We cannot just populate the HashMap via a copy constructor new
         * HashMap<String, MutablePoint>(locations), because only the references
         * to the points would be copied, not the point objects themselves.
         *
    ...
    public synchronized Map<String, MutablePoint> getLocations() {
        /**
         * We cannot just wrap the Map with "unmodifiableMap", because that
         * protects only the collection from modification; it does not prevent
         * callers from modifying the mutable objects stored in it.
         *
         * Use deepCopy to safely copy the map.
         */
    ...
    public synchronized MutablePoint getLocation(final String id) {
        ...
        /**
         * The original point object never gets published as we are using a copy
         * constructor to return a copy of the original point object.
         */
