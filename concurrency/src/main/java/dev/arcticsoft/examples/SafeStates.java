 ...
 * constructor, regardless of how the object is published. Further, any
 * variables that can be reached through a final field of a properly constructed
 * object (such as the elements of a final array of the contents of a HashMap
 * referenced by a final field) are also guaranteed to be visible to other
 * threads.
 *
 * Initialization safety means that SafeStates could be safely published even
 * through unsafe lazy initialization or stashing a reference to a SafeStates in
 * a public static field with no synchronization, even though it uses no
 ...
 * fields. And allowing the object to escape during construction invalidates the
 * initialization-safety guarantee.
 *
 * Initialization safety makes visibility guarantees only for the values that
 * are reachable through final fields as of the time the constructor finished.
...
public class SafeStates {
    ...
    /**
     * For objects with final fields, initialization safety prohibits reordering
     * any part of construction with the initial load of a reference to that
     * object.
     */
    public SafeStates() {
