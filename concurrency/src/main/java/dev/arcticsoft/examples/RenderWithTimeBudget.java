 ...
 *
 * RenderWithTimeBudget shows a typical application of a timed Future.get. It
 * generates a composite web page that contains the requested content plus an
 * advertisement fetched from and ad server. It submits the ad-fetching task to
 * an executor, computes the rest of the page content, and then waits for the ad
 * until its time budget runs out (all the timed methods in java.concurrent
 * package treat negative timeouts as zero).
 *
...
public class RenderWithTimeBudget {
    ...
    public Page renderPageWithAd() throws InterruptedException {
         ...
         * A secondary problem when using timed tasks is to stop them when they
         * run out of time, so they do not waste computing resources by
         * continuing to compute a result that will not be used. If a timed
         * "get" method completes with a TimeoutException, you can cancel the
         * task through the Future.
         */
