 ...
 *
 * ConcurrentPuzzleSolver uses an inner SolverTask class that extends Node and
 * implements Runnable. Most of the work is done in "run" method: evaluating the
 * set of possible next positions, pruning positions already searched,
 * evaluating whether success has yet been achieved (by this task or other
 * task), and submitting unsearched positions to an Executor.
 *
 */
public class ConcurrentPuzzleSolver<P, M> {
     ...
     * instead of the call stack to hold the state of the search -> free of
     * stack size restrictions but can still run out of memory if the set of
     * positions exceeds the available memory.
     */
     ...
     * ConcurrentHashMap for this purpose.
     *
     * This provides thread-safety and avoids the race condition using atomic
     * putIfAbsent method.
     */
    ...
    public ConcurrentPuzzleSolver(final Puzzle<P, M> puzzle) {
        ...
        if (this.exec instanceof ThreadPoolExecutor threadPoolExecutor) {
    ...
    public List<M> solve() throws InterruptedException {
        try {
             ...
             * in ValueLatch blocks until some thread has set the value.
             */
    ...
    protected class SolverTask extends PuzzleNode<P, M> implements Runnable {
        ...
        public void run() {
            /**
             * Each task first consults the solution latch and stops if a
