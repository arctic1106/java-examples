 ...
 *
 * It performs a depth-first search of the puzzle space. It terminates when it
 * finds a solution (which is not necessarily the shortest solution)
 *
 */
public class SequentialPuzzleSolver<P, M> {
    ...
    public List<M> solve() {
        ...
        return this.search(new PuzzleNode<>(pos, null, null));
    }
    ...
    private List<M> search(final PuzzleNode<P, M> node) {
        if (!this.seen.contains(node.pos)) {
            ...
            for (final M move : this.puzzle.legalMoves(node.pos)) {
                ...
                final PuzzleNode<P, M> child = new PuzzleNode<>(pos, move, node);
