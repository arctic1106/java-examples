 ...
 *
 * It lets you provide a thread name, sets a custom link
 * {@link UncaughtExceptionHandler} that writes a message to a Logger, maintains
...
public class MyAppThread extends Thread {
    ...
    public MyAppThread(final Runnable runnable, final String name) {
        ...
        this.setUncaughtExceptionHandler((final Thread t, final Throwable e) -> {
    ...
    public void run() {
        ...
        if (debug) {
            log.log(Level.FINE, "Created {0}", this.getName());
        }
        try {
            ...
            if (debug) {
                log.log(Level.FINE, "Exiting {0}", this.getName());
            }
        }
    }
